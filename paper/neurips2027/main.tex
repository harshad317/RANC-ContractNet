...
invariance compilation rather than AutoML scaler search. Given a training fold, a
declared contract, and implemented checkers, it builds regime cards, records
...
contract and checker support: accepted policies pass checked clauses on stated
support, while failures become recorded downgrades or no-ops. In paired outlier
worlds where identical extremes are either noise or rare signal, correct
contracts improve the targeted failure metric over wrong-contract controls. The
claim is auditable legality under declared invariances and implemented checkers,
not universal predictive dominance or leaderboard status.
\end{abstract}
...
\caption{Evidence map for the contract-compilation claim. Rows are ordered by
claim strength: semantic intervention first, safety/audit evidence second, and
public execution evidence third. Each row names the failure it should expose and
the artifact that must make the result auditable; no row is a universal scaler
leaderboard.}
...
Panel A is the mechanism: contracts constrain policy selection before validation
score. Panel B is the decisive semantic test: changing declared outlier meaning
changes the targeted failure metric. Panel C is the scoped public-data check:
signed OpenML/UCI deltas show where RANC helps or loses after metric-direction
alignment, with full task tables and exclusions retained in the artifact.}
